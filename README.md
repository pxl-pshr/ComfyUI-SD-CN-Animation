...
| **SD-CN Motion Control** | Control flow, zoom, pan, and rotation in Txt2Vid |
...
- `loop_frames` — set to 10-20 to blend the last N frames back toward the first frame for a seamless loop. 0 = disabled
- `color_match` (optional) — histogram reference for color stability: `first_frame` (default, most stable), `previous_frame` (lets colors evolve), or `none`
...
Input frames whose width or height isn't a multiple of 8 (e.g. 854×480) are center-cropped by a few pixels to fit the VAE.
...
>
> For schedules that change the color palette (e.g. forest → ocean), set Txt2Vid's `color_match` to `previous_frame`. The default `first_frame` matches every frame's colors to frame 1, which pulls later prompts back toward the opening palette.
...
|-----------|---------|-------------|
| `occlusion_multiplier` | 10.0 | How much of each frame gets regenerated by SD. Higher = more creative freedom, lower = more temporal coherence |
| `flow_scale` | 1.0 | Multiplier on FloweR's predicted motion. `< 1` = slower, `> 1` = exaggerated |
| `pan_x` | 0.0 | Horizontal drift in output pixels/frame. Positive = pan right |
| `pan_y` | 0.0 | Vertical drift in output pixels/frame. Positive = pan down |
| `zoom` | 1.0 | Per-frame scale factor. Values compound — use small increments |
| `rotate` | 0.0 | Degrees/frame rotation |
...
All values are per-frame, so even subtle settings produce visible effects over 60+ frames. Zoom and rotate are applied as affine transforms on the FloweR prediction, while flow_scale and pan modify the optical flow field directly. Areas that move in from outside the frame (edges when panning, zooming out or rotating) are automatically marked for SD to fill in.
...
MIT (see [`LICENSE`](LICENSE)). RAFT model code under BSD 3-Clause (see [`raft/LICENSE`](raft/LICENSE)).
